...
class Dkv extends CI_Controller {
	...
	public function index($prodi=null,$smt=null,$kelas=null)
        {
            ...
            $data['content'] = 'admin/dkv/pemilih_dkv';
    ...
    function addKelas(){
        ...
        if($jml==null){
            ...
            redirect('admin/pemilih/dkv/','refresh');
        }
        $data = array(
                'id_prodi' => 'dkv',
    ...
    function add_pemilih($prodi=null,$smt=null,$kelas=null){
        if($prodi==null||$smt==null||$kelas==null){
            ...
            redirect('admin/dkv/index/'.$prodi.'/'.$smt.'/'.$kelas);
        }		
        ...
        if($this->input->post('submit')){
            if ($this->form_validation->run() == TRUE) {
                ...
                if($this->ion_auth->register($username,$password,null,$additional,$group)){
                    ...
                    redirect('admin/dkv/add_pemilih/'.$prodi.'/'.$smt.'/'.$kelas);
                }
                else{
                ...
                $data['kelas']		= $kelas;
                $data['smt']		= $smt;
                $data['content']	= 'admin/dkv/add_pemilih';
                $this->load->view('template',$data);
            }
        }
        else{
    ...
    function edit_pemilih($id=null,$prodi=null,$smt=null,$kelas=null){
        ...
        if($users_group){
            ...
            $data['kelas']		= $kelas;
            $data['smt']		= $smt;
            $data['content']	= 'admin/dkv/edit_pemilih';
            $this->load->view('template',$data);
        }
        else{
            $this->session->set_flashdata('info', 'Update Failed!');
            redirect('admin/dkv/edit_pemilih/'.$id.'/'.$prodi.'/'.$smt.'/'.$kelas);
        }
    }
...
/* End of file Dkv.php */
/* Location: ./application/controllers/admin/Dkv.php */