...
class Ti extends CI_Controller {
	...
	public function index($prodi=null,$smt=null,$kelas=null)
	{
		...
		$data['content'] = 'admin/ti/pemilih_ti';
	...
	function add_pemilih($prodi=null,$smt=null,$kelas=null){
		if($prodi==null||$smt==null||$kelas==null){
			$this->session->set_flashdata('info-umum', 'Pilihan Gagal! Silahkan Ulangi Kembali!');
			redirect('admin/ti/index/'.$prodi.'/'.$smt.'/'.$kelas);
		}		
		...
		if($this->input->post('submit')){
			if ($this->form_validation->run() == TRUE) {
				...
				if($this->ion_auth->register($username,$password,null,$additional,$group)){
					...
					redirect('admin/ti/add_pemilih/'.$prodi.'/'.$smt.'/'.$kelas);
				}
				else{
				...
				$data['kelas']		= $kelas;
				$data['smt']		= $smt;
				$data['content']	= 'admin/ti/add_pemilih';
				$this->load->view('template',$data);
			}
		}
		else{
	...
	function edit_pemilih($id=null,$prodi=null,$smt=null,$kelas=null){
		...
		if($users_group){
			...
			$data['kelas']		= $kelas;
			$data['smt']		= $smt;
			$data['content']	= 'admin/ti/edit_pemilih';
			$this->load->view('template',$data);
		}
		else{
			$this->session->set_flashdata('info', 'Update Failed!');
			redirect('admin/ti/edit_pemilih/'.$id.'/'.$prodi.'/'.$smt.'/'.$kelas);
		}
	}
...
/* End of file Ti.php */
/* Location: ./application/controllers/admin/Ti.php */