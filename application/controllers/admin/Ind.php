...
class Ind extends CI_Controller {
	...
	public function index($prodi=null,$smt=null,$kelas=null)
        {
            ...
            $data['content'] = 'admin/ind/pemilih_ind';
    ...
    function addKelas(){
        ...
        if($jml==null){
            ...
            redirect('admin/pemilih/ind/','refresh');
        }
        $data = array(
                'id_prodi' => 'ind',
    ...
    function add_pemilih($prodi=null,$smt=null,$kelas=null){
        if($prodi==null||$smt==null||$kelas==null){
            ...
            redirect('admin/ind/index/'.$prodi.'/'.$smt.'/'.$kelas);
        }       
        ...
        if($this->input->post('submit')){
            if ($this->form_validation->run() == TRUE) {
                ...
                if($this->ion_auth->register($username,$password,null,$additional,$group)){
                    ...
                    redirect('admin/ind/add_pemilih/'.$prodi.'/'.$smt.'/'.$kelas);
                }
                else{
                ...
                $data['kelas']      = $kelas;
                $data['smt']        = $smt;
                $data['content']    = 'admin/ind/add_pemilih';
                $this->load->view('template',$data);
            }
        }
        else{
    ...
    function edit_pemilih($id=null,$prodi=null,$smt=null,$kelas=null){
        ...
        if($users_group){
            ...
            $data['kelas']      = $kelas;
            $data['smt']        = $smt;
            $data['content']    = 'admin/ind/edit_pemilih';
            $this->load->view('template',$data);
        }
        else{
            $this->session->set_flashdata('info', 'Update Failed!');
            redirect('admin/ind/edit_pemilih/'.$id.'/'.$prodi.'/'.$smt.'/'.$kelas);
        }
    }
...
/* End of file Mi.php */
/* Location: ./application/controllers/admin/Mi.php */